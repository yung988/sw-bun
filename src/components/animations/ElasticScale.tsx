 ...
 */
export default function ElasticScale({ children, className = '', scaleRange = [0.8, 1.2] }: ElasticScaleProps) {
  ...
  return (
    <div ref={ref} className={className}>
      <motion.div style={{ scale, rotate: rotateSpring }}>{children}</motion.div>
    </div>
  )
}
