...
export default function SubcategoryTOC({ subcategories }: { subcategories: Subcategory[] }) {
  ...
  return (
    <nav aria-label="Navigace podkategorií" className="space-y-2">
      {subcategories.map((s) => {
        ...
        return (
          <a
            ...
            className={`block rounded-lg px-3 py-2 text-sm transition-colors ${isActive ? 'bg-slate-900 text-white' : 'text-slate-700 hover:bg-slate-50 hover:text-slate-900'}`}
          >
