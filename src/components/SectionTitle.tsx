    ...
    <div className={`space-y-6 ${center ? 'text-center' : 'text-left'}`}>
      {eyebrow && (
        <div className={`flex items-center gap-2 ${center ? 'justify-center' : 'justify-start'}`}>
          ...
          <span className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-medium">{eyebrow}</span>
        </div>
      )}
      ...
      {subtitle && (
        <p className={`text-[15px] md:text-base leading-relaxed text-slate-600 ${center ? 'mx-auto max-w-2xl md:max-w-3xl' : 'max-w-2xl'}`}>
          {subtitle}
        </p>
      )}
    </div>
